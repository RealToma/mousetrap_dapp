...
import Cheez from "../assets/tokens/cheez.png";
import sCheez from "../assets/tokens/cheezstakedc.png";
...
export async function getTokenPrice(tokenId = "cheesedao") {
...
export function getRebaseBlock(currentBlock: number) {
  return EPOCH_INTERVAL - ((currentBlock - 19982257) % EPOCH_INTERVAL) + currentBlock
}
...
function getSCheezTokenImage() {
  return <img src={sCheez} alt="cheez logo" />;
}

export function getCheezTokenImage(w?: number, h?: number) {
  const height = h == null ? "32px" : `${h}px`;
  const width = w == null ? "32px" : `${w}px`;
  return <img src={Cheez} alt="cheez logo" style={{ height, width }} />;
}

export function getTokenImage(name: string) {
  if (name === "ohm") return getCheezTokenImage();
  if (name === "sohm") return getSCheezTokenImage();
}
