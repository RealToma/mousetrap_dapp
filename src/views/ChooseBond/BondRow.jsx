...
export function BondDataCard({ bond }) {
  ...
  return (
    <Slide direction="up" in={true}>
      <Paper id={`${bond.name}--bond`} className="bond-data-card cheez-card">
        ...
        <Link component={NavLink} to={`/bonds/${bond.name}`}>
          <Button variant="outlined" color="primary" fullWidth disabled={!bond.isAvailable[networkId]}>
            <Typography variant="h5">
              {!bond.isAvailable[networkId] ? "Expired" : `Bond ${bond.displayName}`}
            </Typography>
          </Button>
        </Link>
      </Paper>
      ...
      <TableCell>
        <Link component={NavLink} to={`/bonds/${bond.name}`}>
          <Button variant="outlined" color="primary" disabled={!bond.isAvailable[networkId]}>
            <Typography variant="h6">{!bond.isAvailable[networkId] ? "Expired" : `Bond`}</Typography>
          </Button>
        </Link>
      </TableCell>
    </TableRow>
