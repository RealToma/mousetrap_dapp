...
export const Web3ContextProvider: React.FC<{ children: ReactElement }> = ({ children }) => {
  ...
  const [chainID, setChainID] = useState(1666600000);
  ...
  const _checkNetwork = (otherChainID: number): Boolean => {
    if (chainID !== otherChainID) {
      ...
      if (otherChainID === 1666600000 || otherChainID === 4) {
        setChainID(otherChainID);
        otherChainID === 1666600000 ? setUri(getMainnetURI()) : setUri(getTestnetURI());
        return true;
      }
      return false;
    }
        ...
        if (err.code === 4902) {
          try {
            await rawProvider.request({
              ...
              params: [{ chainId: '0x63564C40', rpcUrls:["https://api.harmony.one/"], chainName: 'Harmony Mainnet', blockExplorerUrls: ["https://explorer.harmony.one/"],
              nativeCurrency: {
                  name: "ONE",
                  symbol: "ONE",
                  decimals: 18
            ...
            console.error("Wrong network, please switch to ONE Harmony mainnet");
          }
        }
