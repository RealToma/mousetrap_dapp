...
import { IActionValueAsyncThunk, IChangeApprovalAsyncThunk, IJsonRPCError, IForfeitThunk } from "./interfaces";
...
export const changeStake = createAsyncThunk(
  ...
  async ({ action, value, provider, address, networkID }: IActionValueAsyncThunk, { dispatch }) => {
    ...
    try {
      ...
      const pendingTxnType = action === "stake" ? "staking" : "claim" ? "claiming" : "forfeit" ? "forefeiting" : "unstaking";
...
export const forfeit = createAsyncThunk(
  "stake/forfeit",
  async ({ provider, address, networkID }: IForfeitThunk, { dispatch }) => {
    if (!provider) {
      dispatch(error("Please connect your wallet!"));
      return;
    }
